...
//Rare exceptions to indicate a bug in the _programiv code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_programiv = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_programiv]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.programiv.module"
